...
const emitNewMessageToUser = (payload: { userId: string, conversationId: string, messageInfo: any }) => {
    // This is a placeholder for the actual implementation if it exists elsewhere or needs to be defined
    // Based on message.socket.ts, it seems to be defined there. If it's shared, it should be in a util.
    // For now, I'll keep the logic as it was in the original file (emitting to rider).
};

export const newBidEventHandler = async (socket: Socket, data: any) => {
    const { rideId, amount } = data;
    const { _id: driverId } = socket.payload;

    const ride = await RideRequest.findOne({ _id: rideId });
    if (!ride) {
        throw new SocketError("new-bid", "Ride not found", null);
    }

    const rider = await User.findOne({ _id: ride.riderId });
    if (!rider) {
        throw new SocketError("new-bid", "Rider not found", null);
    }

    const driver = await User.findOne({ _id: driverId });
    if (!driver) {
        throw new SocketError("new-bid", "Driver not found", null);
    }
...
export const acceptBidEventHandler = async (socket: Socket, data: any) => {
    const { rideId, driverId } = data;
    const { _id } = socket.payload;

    const ride = await RideRequest.findOne({ _id: rideId });
    if (!ride) {
        throw new SocketError("accept-bid", "Ride not found", null);
    }
    ...
    try {
        ...
        const driver = await User.findOne({ _id: driverId });
        if (!driver) {
            throw new SocketError("accept-bid", "Driver not found", null);
        }
        ...
        const rider = await User.findOne({ _id: ride.riderId });
        if (!rider) {
            throw new SocketError("accept-bid", "Rider not found", null);
        }
        ...
        throw new SocketError("accept-bid", error.message || "Failed to accept bid", null);
    } finally {
...
export const cancelBidEventHandler = async (socket: Socket, data: any) => {
    const { rideId, driverId } = data;

    const ride = await RideRequest.findOne({ _id: rideId });
    if (!ride) {
        throw new SocketError("cancel-bid", "Ride not found", null);
    }
...
export const pickupRiderEventHandler = async (socket: Socket, data: any) => {
    const { _id } = socket.payload;
    ...
    if (ride.driverId && ride.driverId.toString() !== _id.toString()) {
        throw new SocketError("pickup-rider", "You are not the driver of this ride", null);
    }

    if (ride.status !== "ACCEPTED") {
        throw new SocketError("pickup-rider", "Ride not accepted", null);
    }
    ...
    try {
        ...
        await session.abortTransaction();
        if (error instanceof SocketError) throw error;
        throw new SocketError("pickup-rider", error.message || "Failed to pickup rider", null);
    } finally {
        await session.endSession();
    }
...
export const dropOffRiderEventHandler = async (socket: Socket, data: any) => {
    const { _id } = socket.payload;
    ...
    if (ride.driverId && ride.driverId.toString() !== _id.toString()) {
        throw new SocketError("drop-off-rider", "You are not the driver of this ride", null);
    }
    ...
    try {
        ...
        await session.abortTransaction();
        if (error instanceof SocketError) throw error;
        throw new SocketError("drop-off-rider", error.message || "Failed to drop off rider", null);
    } finally {
        await session.endSession();
    }
...
export const cancelRideEventHandler = async (socket: Socket, data: any) => {
    ...
    if (ride.riderId.toString() !== _id.toString()) {
        throw new SocketError("cancel-ride", "You are not the rider of this ride", null);
    }

    if (ride.status == "ONGOING" || ride.status == "ACCEPTED" || ride.status == "COMPLETED") {
        throw new SocketError("cancel-ride", "Ride is not in the right state to be cancelled", null);
    }
    ...
    try {
        ...
        await session.abortTransaction();
        if (error instanceof SocketError) throw error;
        throw new SocketError("cancel-ride", error.message || "Failed to cancel ride", null);
    } finally {
        await session.endSession();
    }
