...
const rideAcceptedEventHandler = async (socket: Socket, data: any) => {
    ...
    if (user.isEngaged) {
        throw new SocketError("accept-ride", "You are already engaged with another ride", null);
    }
    ...
    if (ride.status !== RideConstants.RIDE_STATUS.PENDING) {
        throw new SocketError("accept-ride", "You can't accept this ride", null);
    }
    ...
    try {
        ...
        throw new SocketError("accept-offer", error.message || "Failed to accept offer", null);
    } finally {
...
const pickupRiderEventHandler = async (socket: Socket, data: any) => {
    ...
    if (ride.status !== "ACCEPTED") {
        throw new SocketError("pickup-rider", `The ride cannot be picked up as it is not in accepted state. Current state: ${ride.status}`, null);
    }
